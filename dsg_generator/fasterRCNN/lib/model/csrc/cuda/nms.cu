...
__global__ void nms_kernel(const int n_boxes, const float nms_overlap_thresh,
  ...
  if (threadIdx.x < col_size) {
    for (int i = 0; i < 5; i++) {
      block_boxes[threadIdx.x * 5 + i] = dev_boxes[(threadsPerBlock * col_start + threadIdx.x) * 5 + i];
    }
  }
  __syncthreads();
...
at::Tensor nms_cuda(const at::Tensor boxes, float nms_overlap_thresh) {
  ...
  AT_CUDA_CHECK(cudaMemcpy(&mask_host[0], mask_dev, sizeof(unsigned long long) * boxes_num * col_blocks, cudaMemcpyDeviceToHost));
