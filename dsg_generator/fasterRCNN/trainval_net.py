...
def get_fixed_args():
    """
    Fixed training configuration replacing argparse.
    """
...
if __name__ == '__main__':
    ...
    if torch.cuda.is_available() and not args.cuda:
        print("WARNING: CUDA is available, but args.cuda is False")
    ...
    config = CONFIG.dsg
    output_dir = args.save_dir + "/object_detector/" + config['data_path'].split("/")[-1]
    ...
    for epoch in range(args.start_epoch, args.max_epochs + 1):
        ...
        save_name = os.path.join(output_dir, 'faster_rcnn_model_{}.pth'.format(epoch))
        save_checkpoint({
