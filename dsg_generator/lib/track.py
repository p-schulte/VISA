...
def clean_bbox(entry):
    ...
    counts = 0
    mapping = {}
    for i in range(int(entry["boxes"][-1,0])+1): # number of images in the batch
        # images in the batch
...
def get_sequence(entry, gt_annotation, matcher, shape, task="sgdet", GENERAL_COARSE_TRACKING=False):
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    if task == "predcls":
        indices = []
    ...
    if not GENERAL_COARSE_TRACKING and task == "sgdet":    
    ...
    for index, img in enumerate(key_frames):
        current_key = int(img.split("_")[1].split(".")[0])
        # video timestamp
        ...
        if len(tracks) > 0:
            ...
            boxes_features = torch.cat([torch.mean(cluster_feature[t.cluster],dim=0,keepdim=True) for t in tracks]).cpu()
            boxes_dists = torch.cat([torch.mean(cluster_dist[t.cluster],dim=0,keepdim=True) for t in tracks]).cpu()
            row_ind, col_ind, cost1, cost2 = matcher({"boxes":norm_pred, "features": pred_features, "dists": pred_dists}, {"boxes":norm_boxes, "features": boxes_features, "dists": boxes_dists})
            for t, (r,c) in enumerate(zip(row_ind, col_ind)):
                ...
                if (cost1[t] < 0.5) or (cost2[t]<0.5):                 
                    ...
                    if task == "sgcls":
                        ...
                        if (pred[r][0]+pred[r][2]>h) or (pred[r][1] + pred[r][3]>w) or (pred[r][0] <0) or (pred[r][1]<0):
                            continue
                    cluster_feature[tracks[c].cluster] = torch.cat([cluster_feature[tracks[c].cluster], final_features[torch.where(frame_id==index)[0][r:r+1]]])                        
                    cluster_dist[tracks[c].cluster] = torch.cat([cluster_dist[tracks[c].cluster], final_dists[torch.where(frame_id==index)[0][r:r+1]]])                        
        ...
        if len(row_ind) < len(pred):
            for j in range(len(pred)):
                if j not in row_ind:
                    ...
                    if task == "sgcls":
                        if (pred[j][0]+pred[j][2]>h) or (pred[j][1] + pred[j][3]>w) or (pred[j][0] <0) or (pred[j][1]<0):
                            cluster_feature.append([])
                            cluster_dist.append([])
                            continue
    ...
    if task == "sgcls":
        ...
        for l in cluster:
            k = l
            if len(k) > 0:
                entry["indices"].append(torch.LongTensor(k).to(device))
    else:
        ...
        entry["indices"] = [torch.LongTensor(l).to(device) for l in new_cluster]



def get_sequence_simple(entry, gt_annotation):

    # initialize each object as an own tracklet
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    indices = []
    prev_pos = []
    for i, box in enumerate(entry['boxes'][entry['boxes'][:, 0] == 0]):
        indices.append(torch.LongTensor([i]))
        prev_pos.append(box[1:])

    # iterate over the frames and add the object to the tracklet with the smallest distance
    for f in range(1, len(gt_annotation)):
        
        first_idx = (entry['boxes'][:, 0] == f).nonzero(as_tuple=True)[0][0].item()
        
        # extract current positions of objects
        current_pos = []
        for box in entry['boxes'][entry['boxes'][:, 0] == f]:
            current_pos.append(box[1:])


        # calculate distance between current and previous positions and assign to tracklet with smallest distance
        out_of_range = []
        for i, pos in enumerate(current_pos):
            dists = []
            for prev in prev_pos:
                dists.append((abs(pos - prev)).sum())
            min_dist = min(dists)
            min_index = dists.index(min_dist)
            if min_dist < 10:
                indices[min_index] = torch.cat((indices[min_index], torch.LongTensor([i+first_idx])))
            else:
                out_of_range.append(i+first_idx)
        if len(out_of_range) != 1:
            print("Out of range objects: ", out_of_range)
        # add out of range objects to tracklets with smallest amount of objects
        for i in out_of_range:
            min_index = min(range(len(indices)), key=lambda x: len(indices[x]))
            
            indices[min_index] = torch.cat((indices[min_index], torch.LongTensor([i])))
            
        # reset previous positions
        prev_pos = []
        for idx in indices:
            if len(idx) > 0:
                prev_pos.append(entry['boxes'][idx[-1]][1:])
        
    
    # convert indices to tensors and save them in the entry
    indices = [vec.to(device) for vec in indices]
    entry['indices'] = indices