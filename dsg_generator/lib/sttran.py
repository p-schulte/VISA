...
class PositionalEncoding(nn.Module):
    ...
    def forward(self, x: Tensor, indices=None) -> Tensor:
        ...
        try:
            ...
            print("Error in PositionalEncoding: ", e)
            print(indices)
            raise e
...
class ObjectClassifier(nn.Module):
    ...
    def __init__(self, mode='sgdet', obj_classes=None):
        ...
        self.positional_encoder = PositionalEncoding(d_model, 0.1, 1000)
    ...
    def get_edges(self, entry):
        ...
        return torch.cat([edges, torch.flip(edges, [0])], 1).to(self.device)
             
    def forward(self, entry):

        if self.mode  == 'predcls':
            print("this shouldn not be (predcls)!")
            import sys; sys.exit(0)
        elif self.mode == 'sgcls': 
            print("this shouldn not be (sgcls)!")
            import sys; sys.exit(0)
        else: 
            ...
            if self.training:
                ...
                if len(indices)>1:
                    ...
                    obj_ = self.encoder_tran(self.positional_encoder(sequence_features, pos_index),src_key_padding_mask=masks.to(self.device))
                ...
                final_feats = []
                final_labels = []
                for i in range(b): # b is the amount of frames in the batch
                    # images in the batch
                    scores = entry['distribution'][entry['boxes'][:, 0] == i]
                ...
                if entry['distribution'].size()[-1] != len(self.classes)-1: # TODO: this was fixed, maybe it doesn't work for everything
                ...
                entry["pred_labels"] = entry["labels"] # change this back
                
                im_idx = []  # frame-id of the predicted relations
                pair = [] # pair of indices of the detected objects in the relations
                for i in range(b):
                    box_indices = torch.where(entry['boxes'][:, 0] == i)[0]
                    for a in box_indices:
...
class STTran(nn.Module):
    ...
    def __init__(self, mode='sgdet',
                 spatial_class_num=None, obj_classes=None, rel_classes=None,
        ...
        d_model=1936
        self.positional_encoder = PositionalEncoding(d_model, max_len=1000)
        # temporal encoder
    ...
    def forward(self, entry):
        ...
        rel_ = self.local_transformer(frame_features, src_key_padding_mask=masks.to(self.device))
        rel_features = torch.cat([rel_[i,:len(index)] for i, index in enumerate(frames)])
 
        if self.skip_temporal: # Ablation study: only spatial message passing
        ...
        rel_ = self.global_transformer(self.positional_encoder(sequence_features, pos_index),src_key_padding_mask=masks.to(self.device)) # When OOM, this line is the problem!
