...
class ViTEncoderLayer(nn.Module):
    def __init__(self, d_model=1936, nhead=4, dim_feedforward=2048, dropout=0.1, batch_first=True):
            super().__init__()
            embed_dim = d_model
    ...
    def forward(self, src, input_key_padding_mask=None):
            # local attention
            if self.batch_first:
            ...
            if self.batch_first:
                src = src.permute(1,0,2)
                local_attention_weights = local_attention_weights.permute(1,0,2)
            return src, local_attention_weights
        
class ViTEncoder(nn.Module):

    def __init__(self, encoder_layer, num_layers, return_attention = False):
        super().__init__()
    ...
    def forward(self, input, src_key_padding_mask=None,):
        output = input
        input_key_padding_mask = src_key_padding_mask
        weights = torch.zeros([self.num_layers, output.shape[1], output.shape[0], output.shape[1]]).to(output.device)
...
class transformer(nn.Module):
    ...
    def forward(self, features, im_idx):
        ...
        for j in range(b - 1):
            ...
            idx_plus[:torch.sum((im_idx == j) + (im_idx == j + 1)), j] = rel_idx[(im_idx == j) + (im_idx == j + 1)]
