...
def generate_random_pddl_problem(pddl_dir, problem_name="random_problem.pddl", num_blocks=7, num_piles=7):
    """
    ...
    num_ontable = random.randint(1, min(num_blocks, num_piles))  # Random number of blocks on table
    ...
    holding_block = None
    if random.random() > 0.5:  # 50% chance the robot is holding a block
    ...
    with open(problem_file_path, "w") as f:
        ...
        f.write("\n\n        ; action literals\n")
        for block in blocks:
            f.write(f"        (pickup {block})\n")
            f.write(f"        (putdown {block})\n")
            f.write(f"        (unstack {block})\n")
            for other in blocks:
                if block != other:
                    f.write(f"        (stack {block} {other})\n")

        f.write("    )\n")
        
        # Set a random goal
        goal_blocks = random.sample(blocks, min(len(blocks) - 1, 3))
        f.write("    (:goal (and\n")
