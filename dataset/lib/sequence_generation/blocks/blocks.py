...
from lib.rendering.blocks import blocks_render
from lib.rendering.utils import PileTracker

def register_blocks_env(initialize_randomly=False, num_blocks=7, num_piles=7):
    """
    ...
    ENV_NAME = "MyCustomBlocksEnv-v0"
    PDDL_DOMAIN_DIR = "pddl_files/blocks"
    PDDL_PROBLEMS_DIR = "pddl_files/blocks/problems"

    if initialize_randomly:
        # Generate a random PDDL problem
        PDDL_PROBLEMS_DIR = "pddl_files/blocks/random_problems"
        import lib.sequence_generation.blocks.random_state as random_state
        import os
        random_state.generate_random_pddl_problem(PDDL_PROBLEMS_DIR, num_blocks=num_blocks, num_piles=num_piles)

    # Register the environment
    register(
        id=ENV_NAME,  
        entry_point="pddlgym.core:PDDLEnv",
        kwargs={"domain_file": f"{PDDL_DOMAIN_DIR}/domain.pddl", "problem_dir": f"{PDDL_PROBLEMS_DIR}/", 'render' : blocks_render.render},
    )
...
def create_fresh_env(config, test_set=False):
    ...
    env = register_blocks_env(initialize_randomly=True, num_blocks=NUM_BLOCKS, num_piles=MAX_PILES)
    obs, _ = env.reset()


    # Randomize colors
    from lib.rendering.blocks import blocks_render
    blocks_render._block_name_to_color = {}
