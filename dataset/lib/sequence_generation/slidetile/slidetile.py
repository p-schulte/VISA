...
from lib.rendering.slidetile import slidetile_render

def register_slidetile_env(initialize_randomly=False, size=3):
    """
    ...
    ENV_NAME = "MyCustomSlidetileEnv-v0"
    PDDL_DOMAIN_DIR = "pddl_files/slidetile"
    PDDL_PROBLEMS_DIR = "pddl_files/slidetile/problems"

    if initialize_randomly:
        # Generate a random PDDL problem
        PDDL_PROBLEMS_DIR = "pddl_files/slidetile/random_problems"
        import lib.sequence_generation.slidetile.random_state as random_state
        import os
        random_state.generate_random_pddl_problem(PDDL_PROBLEMS_DIR, size=size)
    ...
    register(
        ...
        kwargs={"domain_file": f"{PDDL_DOMAIN_DIR}/domain.pddl", "problem_dir": f"{PDDL_PROBLEMS_DIR}/", 'render' : slidetile_render.render},
    )
...
def create_fresh_env(config, test_set=False):
    ...
    env = register_slidetile_env(initialize_randomly=True, size=size)
    obs, _ = env.reset()
