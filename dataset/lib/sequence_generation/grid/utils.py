...
def detect_action(prev_state, curr_state):
    ...
    if prev_l[op1_x, op1_y] == 2:
        ...
        #     return "push_no_goal", f"p{op2_y+1 + 5*(op2_x)}:position"
        # else:
        #     return "push", f"p{op2_y+1 + 5*(op2_x)}:position"
        return "push_box", f"p{op2_y+1 + 5*(op2_x)}:position"
    else:
        return f"move_{direction}", f"p{op1_y+1 + 5*(op1_x)}:position"
