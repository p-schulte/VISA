...
from lib.rendering.logistics import logistics_render

def register_logistics_env(initialize_randomly=False, num_blocks=7):
    """
    ...
    ENV_NAME = "MyCustomLogisticsEnv-v0"
    PDDL_DOMAIN_DIR = "pddl_files/logistics"
    PDDL_PROBLEMS_DIR = "pddl_files/logistics/problems"

    if initialize_randomly:
        # Generate a random PDDL problem
        PDDL_PROBLEMS_DIR = "pddl_files/logistics/random_problems"
        import lib.sequence_generation.logistics.random_state as random_state
        import os
        random_state.generate_random_pddl_problem(PDDL_PROBLEMS_DIR, num_packages=num_blocks)
    ...
    register(
        ...
        kwargs={"domain_file": f"{PDDL_DOMAIN_DIR}/domain.pddl", "problem_dir": f"{PDDL_PROBLEMS_DIR}/", 'render' : logistics_render.render},
    )
...
def create_fresh_env(config, test_set=False):
    ...
    env = register_logistics_env(initialize_randomly=True, num_blocks=NUM_BLOCKS)
    obs, _ = env.reset()
