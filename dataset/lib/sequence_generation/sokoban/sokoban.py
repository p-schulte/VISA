...
from lib.rendering.sokoban import sokoban_render

def register_sokoban_env(initialize_randomly=False, size=3, num_stones=2):
    """
    ...
    ENV_NAME = "MyCustomSokobanEnv-v0"
    PDDL_DOMAIN_DIR = "pddl_files/sokoban"
    PDDL_PROBLEMS_DIR = "pddl_files/sokoban/random_problems"
    ...
    while not done:
        try:
            ...
            if initialize_randomly:
                ...
                PDDL_PROBLEMS_DIR = "pddl_files/sokoban/random_problems"
                import lib.sequence_generation.sokoban.random_state as random_state
                import os
    ...
    register(
        ...
        kwargs={"domain_file": f"{PDDL_DOMAIN_DIR}/domain.pddl", "problem_dir": f"{PDDL_PROBLEMS_DIR}/", 'render': sokoban_render.render},
    )
...
def create_fresh_env(config, test_set=False):
    ...
    env = register_sokoban_env(initialize_randomly=True, size=size, num_stones=NUM_STONES)
    obs, _ = env.reset()
