...
class JSONThings:
    ...
    def __init__(self, trace_file_path, add_statics):
        ...
        self.trace = self.parse_trace(trace_file_path, max_traces=500)
    ...
    def infer_effects(self):
        ...
        for act_pos, state_action in enumerate(self.trace[:-1]):
            ...
            if state_action['action_name'] not in number_of_effects:
                ...
                if current_effects.keys() != number_of_effects[state_action['action_name']].keys():
                    raise NotImplementedError
                for f in current_effects:
                    if len(current_effects[f][0]) != number_of_effects[state_action['action_name']][f][0]:
                        print(act_pos, f)
                        print(current_effects[f][0], number_of_effects[state_action['action_name']][f][0])
                        print('action_name', state_action['action_name'])
                        raise NotImplementedError
                    if len(current_effects[f][1]) != number_of_effects[state_action['action_name']][f][1]:
                        print(act_pos, f)
                        print(current_effects[f][1], number_of_effects[state_action['action_name']][f][1])
                        print('action_name', state_action['action_name'])
                        raise NotImplementedError
